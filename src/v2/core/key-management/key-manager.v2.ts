import { KeyManagerConfig, KeyPair, KeyRotationState, KeyManagerStatus, KeyValidationResult } from '../../../core/key-management/types/key-manager.types';
...
export class KeyManagerV2 {
  ...
  public async getDecryptionKeys(): Promise<KeyPair[]> {
    ...
    if (this.rotationState.previousKeys && this.rotationService.isInGracePeriod(this.rotationState)) {
  ...
  public async getStatus(): Promise<KeyManagerStatus> {
    const validation = this.currentKeys ? await this.lifecycleService.validateKeys(this.currentKeys) : { isValid: false, hasExpired: true };
