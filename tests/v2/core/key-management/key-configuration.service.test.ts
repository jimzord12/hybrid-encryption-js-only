...
import { KeyConfigurationService } from '../../../../src/v2/core/key-management/key-configuration.service';
...
describe('KeyConfigurationService', () => {
  ...
  describe('validateConfig', () => {
    ...
    it('should throw for non-positive key expiry months', () => {
      ...
      expect(() => configService.validateConfig(invalidConfig)).toThrow('Key expiry months must be positive');
    });

    it('should throw for a negative rotation grace period', () => {
      const invalidConfig = { ...TEST_CONFIG_V2, rotationGracePeriod: -1 };
      expect(() => configService.validateConfig(invalidConfig)).toThrow('Rotation grace period cannot be negative');
    });

    it('should throw for an empty cert path', () => {
      const invalidConfig = { ...TEST_CONFIG_V2, certPath: '' };
      expect(() => configService.validateConfig(invalidConfig)).toThrow('Certificate path cannot be empty');
    });

    it('should throw for a cert path with path traversal', () => {
      const invalidConfig = { ...TEST_CONFIG_V2, certPath: '../unsafe' };
      expect(() => configService.validateConfig(invalidConfig as any)).toThrow('Certificate path contains unsafe path traversal patterns');
    });
  });
});
