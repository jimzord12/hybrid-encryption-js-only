...
describe('MlKemKeyProvider', () => {
  ...
  describe('validateKeyPair', () => {
    ...
    it('should reject key pair with missing public key', () => {
      ...
      expect(result.errors).toContain('Key pair is missing public or secret key');
    });
    ...
    it('should reject key pair with non-Uint8Array public key', () => {
      ...
      expect(result.ok).toBe(false);
      expect(result.errors).toContain('Key pair keys must be Uint8Array');
    });

    it('should reject key pair with non-Uint8Array secret key', () => {
      const invalidKeyPair = {
        ...validKeyPair,
        secretKey: Buffer.from('buffer-not-uint8array') as any,
      };
    ...
    it('should reject NORMAL preset key pair with wrong public key size', () => {
      ...
      expect(result.errors).toContain(
        'Key pair sizes do not match expected sizes: publicKey=1000/1184, secretKey=2400/2400',
      );
    });
    ...
    it('should reject NORMAL preset key pair with wrong secret key size', () => {
      const invalidKeyPair = {
        ...validKeyPair,
        secretKey: new Uint8Array(2000), // Wrong size
      };
      ...
      expect(result.ok).toBe(false);
      expect(result.errors).toContain(
        'Key pair sizes do not match expected sizes: publicKey=1184/1184, secretKey=2000/2400',
      );
    });
    ...
    it('should reject HIGH_SECURITY preset key pair with wrong sizes', () => {
      ...
      expect(result.ok).toBe(false);
      expect(result.errors).toContain(
        'Key pair sizes do not match expected sizes: publicKey=1184/1568, secretKey=3168/3168',
      );
    });
    ...
    it('should accumulate multiple validation errors', () => {
      const invalidKeyPair = {
        ...validKeyPair,
        publicKey: null as any,
        secretKey: 'not-uint8array' as any,
        metadata: {
          ...validKeyPair.metadata,
          createdAt: null as any,
          version: null as any,
